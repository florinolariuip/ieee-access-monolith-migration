...
on isolated structural alignment by 13.4 percentage points. Almost
the entire gap concentrates in four dimensions (D02, D06, D09, D13
in Table~\ref{tab:per-dim}), all four of which were named explicitly
in the baseline prompt. The pattern is interpretable. When asked
for YARP, the LLM produces YARP. When asked for HasDefaultSchema
isolation, it produces HasDefaultSchema isolation. When asked for
.NET~10 Dockerfiles, it writes those. The \texttt{/refactor} skill
prompt, by contrast, does not enumerate these specific
implementations; it leaves the model free to choose architecturally
defensible alternatives, and the model does so consistently.
...
We report this as a 10\% (1 of 10) skill-invocation failure rate,
recorded honestly in
\texttt{experiments/runs/KNOWN\_RUN\_ISSUES.md} and excluded from
the alignment mean (counted toward attempted runs and toward the
failure rate). The likely cause is a race condition in skill
discovery when non-interactive sessions are launched in quick
succession. We did not retry within the protocol; reporting the
failure rate honestly is more useful than presenting a re-run
sample.
...
The four research questions answered separately produce a single
unified picture. Treated as a tool for generating a single
decomposition, the skill-mediated workflow underperforms a raw
LLM with an explicit prompt. Treated as a wrapper around the LLM
that the practitioner interacts with by typing one slash command
per stage, with persistent intermediate artefacts and gated
commits, the workflow provides something the raw LLM does not.
