...
one premature-exit failure observed out of five attempts. We did
not detect a significant difference in single-shot alignment
between the skill-mediated workflow and the matched-specificity
baseline ($t{=}-1.77$, $p \approx 0.11$, $N{=}9$ vs.\ $N{=}4$).
The original 13.4-percentage-point skill-vs-baseline gap therefore
decomposes into a 10.3-point prompt-specificity effect
($t{=}-6.40$, $p<0.001$) and a 3.2-point skill-abstraction
effect that does not reach significance at $\alpha{=}0.05$. On
...
findings honestly. We did not detect a significant difference
between the skill abstraction and a vague-prompt baseline on
single-shot decomposition quality ($p \approx 0.11$, $N{=}9$ vs.\
$N{=}4$); both fall well below a fully prompt-engineered baseline.
The skill abstraction removes the prompt-engineering burden, and
the two conditions exhibit qualitatively different failure modes
(skill-invocation in the wrapper case, premature exit when the
prompt is vague), though our sample is too small to rank the
underlying failure rates with confidence. The skill abstraction
also provides composability across the full SDLC, artefact-mediated
state, and pre-commit gating. The wrapper's value is not better
LLM output; it is ergonomics and qualitative reliability properties
at the cost of a few percentage points of single-shot alignment. We publish skill scripts, both
...
\begin{enumerate}
  ...
  \item Two baseline conditions that decompose the skill-vs-baseline
        ...
        skill-abstraction effect that does not reach significance
        ($p \approx 0.11$). The skill and minimal-prompt conditions
        exhibit qualitatively different failure modes
        (skill-invocation vs.\ premature-exit), one failure observed
        in each, with overlapping Wilson confidence intervals on
...
\label{sec:method-subjects}

The study uses two monolithic .NET codebases. \textit{PetRescue} is
the \textbf{primary case}, on which the four research questions are
answered with full $N{=}10$ protocols. \textit{eShopOnWeb} is a
\textbf{transfer case}, on which a smaller $N{=}3$ protocol checks
whether the primary findings generalise across codebases.
...
\textit{eShopOnWeb} is Microsoft's monolithic e-commerce reference
application, targeting .NET~8, with bounded contexts for Catalog,
Basket, Ordering, and Identity. It is substantially larger than
PetRescue (roughly three times the source line count) and sits in
a domain that the LLM has likely encountered extensively in
pre-training. Critically for the experimental design, Microsoft
maintains an actively-developed microservices reference of the
same domain (\texttt{dotnet/eShop}), giving us a third-party
ground truth against which to score our rubric. We pin the
eShopOnWeb commit in the environment manifest.

We do not claim that the decomposition behaviour observed on these
two codebases transfers to larger codebases, other languages, or
other LLMs without re-evaluation.
...
\textit{Primary case (PetRescue).}
\begin{description}
  \item[Skill-mediated /refactor:] $N=10$ repeated runs. Each run
        starts from a fresh \texttt{rsync} copy of Slice~1 in a
        per-run scratch directory. The user-supplied prompt is
        identical across all runs and consists of one instruction:
        invoke \texttt{/refactor}, proceed without further
        confirmation. Wall-clock time is measured from the moment
        \texttt{claude} is invoked to the moment it exits; API
        latency and human invocation overhead are included in the
        figure.
  \item[Fully-specified baseline:] $N=10$ runs of the same model on
        the same input with \texttt{--disable-slash-commands}. The
        prompt explicitly asks for the same deliverables ---
        per-service projects, YARP-style gateway, schema-isolated
        DbContexts, docker-compose, README, ARCHITECTURE --- so
        that the comparison is between skill-mediated workflow and
        a fully-specified raw-LLM instruction.
  \item[Matched-specificity baseline:] $N=5$ runs of the same model
        on the same input with \texttt{--disable-slash-commands},
        but a deliberately vague single-sentence prompt
        (\textit{``Decompose this .NET monolith into microservices
        and make it deployable.''}). This third condition isolates
        prompt-specificity effect from skill-abstraction effect by
        controlling against an under-specified raw-LLM instruction.
  \item[Full quality-gated cycle:] $N=5$ runs of a feature-addition
        scenario invoking \texttt{/write\_code} $\to$
        \texttt{/write\_unit\_tests} $\to$
        \texttt{/write\_int\_tests} $\to$ \texttt{/review} $\to$
        \texttt{/check\_security} $\to$ \texttt{/fix} $\to$
        \texttt{/publish}.
  \item[Security audit:] one sweep over a 20-file matched-pair
        corpus (\S\ref{sec:method-security}) on which
        \texttt{/check\_security} is invoked once per file in a
        sandboxed scratch directory.
\end{description}

\textit{Transfer case (eShopOnWeb).}
\begin{description}
  \item[Skill-mediated /refactor:] $N=3$ runs with the same
        invocation pattern as the primary case (\texttt{/refactor},
        proceed without confirmation). Smaller $N$ than the primary
        case because the goal is transfer detection, not powered
        repetition; we describe the resulting statistical
        limitations explicitly in \S\ref{sec:results-transfer}.
  \item[Fully-specified baseline:] $N=3$ runs with
        \texttt{--disable-slash-commands} and a prompt that
        enumerates the eShop bounded contexts (Catalog, Basket,
        Ordering, Identity), mirroring the structure of the
        primary case's fully-specified baseline.
\end{description}

\subsection{Pre-registered structural rubrics}
\label{sec:method-rubrics}

Both cases use a 20-dimension structural rubric, pre-registered and
frozen before any \texttt{/refactor} run was scored. Each dimension
has a plain-English statement and a mechanical verifier (a Python
function that reads the repository and returns a binary
aligned/not-aligned verdict with textual evidence). Verifier
implementations may be patched to fix mechanical bugs --- regexes
that do not match their own rubric statement --- but the rubric
itself is frozen. Every such patch is recorded in
\texttt{experiments/scoring/VERIFIER\_CHANGELOG.md} with the
before/after evidence on a regression input.

The primary-case rubric (\texttt{experiments/dimensions.json}) is
derived from the second author's hand-engineered Slice~2 reference,
which was constructed independently of and prior to the
\texttt{/refactor} skill. The transfer-case rubric
(\texttt{experiments/dimensions\_eshop.json}) is derived not from a
reference authored by us but from Microsoft's actively-maintained
\texttt{dotnet/eShop} microservices repository. Using a third-party
reference for the second rubric reduces the risk that the rubric
implicitly encodes choices the skill scripts were tuned to satisfy.
We scored Microsoft's reference against our eShop rubric as a
sanity check: it satisfies $15/20$ dimensions, which we interpret
as a ceiling against which to read the experimental results.
...
\begin{itemize}
  \item Skill vs.\ matched-specificity baseline:
        $t{=}-1.77$, $df{=}9.1$, $p \approx 0.11$ --- no
        significant difference detected at $\alpha{=}0.05$. The
        small sample sizes ($N{=}9$ vs.\ $N{=}4$) limit statistical
        power; we report this as failure to reject the null, not
  ...
  \item Skill vs.\ fully-specified baseline:
        ...
        --- the original headline gap, additive over the previous
        two effects.
\end{itemize}

Of the original 13.4-percentage-point gap, 10.3 points are
attributable to prompt specificity --- when a practitioner names
YARP, the LLM produces YARP; when they name
\texttt{HasDefaultSchema}, the LLM produces that. The remaining
3.2 points are a skill-abstraction effect that does not reach
significance with the sample sizes used here. We do not claim the
skill and vague-prompt conditions are equivalent; we report only
that we could not distinguish them with this sample. What is more
clearly observable in the data is the difference in practitioner
effort: the skill condition is invoked by typing \texttt{/refactor}
once, while the fully-specified baseline requires a 100-word prompt
enumerating eight intended deliverables.

The failure-mode observations are reported as qualitative findings
rather than as a ranked reliability claim. The skill condition had
one skill-invocation failure (\S\ref{sec:ablation-failures}), a
1/10 occurrence (Wilson 95\% CI [1.8\%, 40.4\%]). The minimal-prompt
condition had one premature-exit failure --- the model stated a
plan and exited without executing --- a 1/5 occurrence (Wilson 95\%
CI [3.6\%, 62.4\%]). The fully-specified condition had no observed
failures. The Wilson intervals on the first two conditions overlap
substantially, so our sample is too small to rank their underlying
reliability with confidence. What we can say is that the two
failure modes are qualitatively distinct, and that the
fully-specified-prompt condition exhibited neither in our sample.
...
The transfer case --- introduced in
\S\ref{sec:method-subjects}, with its rubric described in
\S\ref{sec:method-rubrics} and its run protocol in
\S\ref{sec:method-runs} --- tests whether the headline pattern
observed on PetRescue (skill-mediated workflow underperforming a
fully-prompted raw LLM by approximately 13 percentage points)
generalises to a substantially different codebase. Microsoft's
\textit{eShopOnWeb} is a .NET~8 e-commerce monolith roughly three
times the size of PetRescue, sitting in a domain the model has seen
extensively in pre-training. We use Microsoft's active
\texttt{dotnet/eShop} microservices reference as the third-party
ground truth from which the eShop rubric is derived; that reference
scores 15 of 20 against our rubric, establishing a ceiling. The
protocol is intentionally smaller than the primary case ($N{=}3$
each): the goal is transfer detection, not a second fully-powered
experiment.
...
precise: on single-shot decomposition alignment, we could not
distinguish the skill abstraction from an unprompted vague-request
baseline ($p \approx 0.11$), and both fall well below a fully
prompt-engineered request. The 13.4-point gap to the
fully-specified baseline decomposes into a 10.3-point
prompt-specificity effect (significant) and a 3.2-point
skill-abstraction effect that does not reach significance. This
is the headline result.

What earns the wrapper its place, on our data, is the absence of
required prompt engineering rather than higher alignment. The
vague-prompt condition produces statistically similar single-shot
alignment but exhibited a premature-exit failure mode in our sample
(1 of 5 attempts); the skill condition produced one failure of a
different kind (skill invocation, 1 of 10 attempts); the
fully-specified condition produced no failures, but only after a
practitioner wrote a 100-word prompt enumerating the deliverables.
Each condition trades practitioner effort against the kinds of
failures the LLM can drift into. The skill abstraction lets the
practitioner type one slash command and accept the failure profile
the skill imposes; the fully-specified baseline lets the
practitioner front-load the prompt-engineering effort and avoid
both failure modes in our sample. The remainder of this section
discusses what the wrapper provides beyond this single-shot
view.
...
trade-off concrete on alignment: a practitioner who does not write
the detailed prompt drops from 74.0\% to 63.8\% alignment, and the
skill-mediated workflow scores 60.6\% --- a difference from the
vague-prompt baseline that we could not distinguish from sampling
noise on this sample. The two short-prompt conditions also
exhibited one failure each, of qualitatively different kinds.

The skills also impose a measurable cost. We observed one
skill-invocation failure in ten attempts (Wilson 95\% CI [1.8\%,
40.4\%]). We measured a 9-minute wall-clock mean and \$1.50 per
run in model usage. These are not free.
