...
On PetRescue, ten repeated runs of the automated decomposition scored
a mean of 12.11 of 20 (60.6\%, $\sigma{=}3.9\%$) on a pre-registered
twenty-dimension structural rubric, with a 10\% skill-invocation
failure rate. Ten matched baseline runs in which the same model
received the same input without the skill scripts scored
14.80 of 20 (74.0\%, $\sigma{=}3.2\%$, Welch $t{=}-8.19$,
$p \approx 5{\times}10^{-7}$): the raw LLM with an explicit prompt
\emph{outperforms} the skill-mediated workflow on isolated
structural alignment. On eShopOnWeb, three skill-mediated runs scored
13.67 of 20 (68.3\%) and three baseline runs scored 16.33 of 20
(81.7\%), a 13.3 percentage-point gap nearly identical to PetRescue's
13.4-point gap --- the pattern transfers across codebases. The
...
findings honestly: the skill abstraction does not improve single-shot
decomposition quality, but it provides composability across the full
SDLC, artefact-mediated state, pre-commit gating, and lowered prompt
engineering effort. The skills' value lies in the wrapper, not in
better LLM output. We publish skill scripts, both codebases, run
logs, scoring code, and the injection corpus to enable reproduction
under the same configuration, and we discuss the limits of that
scope.
\end{abstract}
...
We treat this as the strongest evidence in the paper that the
headline finding is not a PetRescue artefact. It is also evidence
the paper deliberately stops short of: two codebases, one model,
one prompt-engineer (the authors). A reviewer who wants stronger
generalisation should expect us to point them at the public
artefacts and the harness, both of which are ready to be re-run on
a third codebase by anyone who wishes to do so.
...
13-percentage-point skill-vs-baseline gap replicates across the
two codebases to within 0.1 percentage points, which is more
robust than the original PetRescue-only design and which the
\S\ref{sec:results-transfer} transfer check makes visible. We
still make no claim that the workflow's behaviour transfers to
other languages, other LLMs, or substantially larger codebases
without re-evaluation. We invite re-evaluation on additional
